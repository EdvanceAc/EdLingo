...
class ProgressionService {
   ...
   * @param {number} requiredScore - Minimum required score
   * @returns {Promise<boolean>} Whether quizzes are completed
   */
  async checkQuizCompletion(userId, courseId, requiredScore = 80) {
    try {
      const quizzes = await run_mcp('mcp.config.usrlocalmcp.Postgrest', 'postgrestRequest', {
        method: 'GET',
        path: `/assessments?course_id=eq.${courseId}&user_id=eq.${userId}&select=results`
      });
      return quizzes.every(q => q.results.score >= requiredScore);
    } catch (error) {
  ...
  /**
   * Check if user has met conversation time requirement
   * @param {string} userId - User ID
   * @param {number} requiredMinutes - Required conversation minutes
   * @returns {Promise<boolean>} Whether requirement is met
   */
  async checkConversationMinutes(userId, requiredMinutes) {
    try {
      const progress = await run_mcp('mcp.config.usrlocalmcp.Postgrest', 'postgrestRequest', {
        method: 'GET',
        path: `/user_progress?user_id=eq.${userId}&select=conversation_time`
      });
      return progress[0]?.conversation_time >= requiredMinutes || false;
    } catch (error) {
      console.error('Error checking conversation minutes:', error);
      return false;
    }
  }

  /**
   * Unlock content if all requirements are met
   * @param {string} userId - User ID
   * @param {string} contentId - Content ID to unlock
   * @returns {Promise<boolean>} Whether content was unlocked
   */
  async unlockContentIfEligible(userId, contentId) {
    try {
      // Get requirements for content (assuming stored in progression_rules or similar)
      const requirements = await this.getProgressionRules(contentId);
      let allMet = true;

      for (const req of requirements) {
        if (req.rule_type === 'quiz_completion') {
          allMet = allMet && await this.checkQuizCompletion(userId, req.config.course_id, req.config.required_score);
        } else if (req.rule_type === 'conversation_requirement') {
          allMet = allMet && await this.checkConversationMinutes(userId, req.config.min_minutes);
        }
      ...
      if (allMet) {
        await run_mcp('mcp.config.usrlocalmcp.Postgrest', 'postgrestRequest', {
          method: 'PATCH',
          path: `/user_progress?user_id=eq.${userId}&content_id=eq.${contentId}`,
